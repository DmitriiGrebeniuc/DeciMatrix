...
export function getWinnerExplanation(
  ...
  if (strongestCriteria.length === 0) {
    return 'Этот вариант выиграл, но вклад критериев пока недостаточно выражен.';
  }

  return `Этот вариант выиграл, потому что лучше всего совпал с самыми важными критериями: ${strongestCriteria.join(
    ', '
  )}.`;
}
